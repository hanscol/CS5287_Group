...
# suit the needs of the assignment #5
#
    ...
    def do_work (self):
        ...
        for items in content:
            ...
            for i in range(len(items)):
                sumWork = sumWork + float(items[i][1])
                sumLoad = sumLoad + float(items[i][2])
                count = count + int(items[i][3])

            avgWork = sumWork / float(count)
            avgLoad = sumLoad / float(count)

            # The [0]'th entry of each of the entries of the second level
            # list is the unique word. We just use the first one and dump it
            # into our list
            key_val_list.append ({'token': items[0][0], 'avgWork': avgWork, 'avgLoad': avgLoad})
