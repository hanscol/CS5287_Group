...
#/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
# @NOTE@: changes will be needed here for assignment #5
#
# The structure of the barrier sink will remain same. However,
# since the intermediate key and val is may be different, you
        ...
        while (i < args['cond']):
            ...
            if (args['op'] == "workers_up"):
                ...
                for j in range (len(map_resp)):
                    map_file.write (map_resp[j]['token'] + str (",") + \
                            str(map_resp[j]['sumWork']) + str(",") + \
                            str(map_resp[j]['sumLoad']) + str(",") + \
                            str(map_resp[j]['count']) + str("\n"))
                ...
                for j in range (len(reduce_resp)):
                    reduce_file.write (reduce_resp[j]['token'] + str (",") + \
                            str(reduce_resp[j]['avgWork']) + str(",") + \
                            str(reduce_resp[j]['avgLoad']) + str("\n"))
...
class MR_Framework ():
    ...
    def distribute_map_tasks (self):
        ...
        try:
            ...
            #don't want to have to count lines, we know 100M lines exist in the file
            
            #num_lines = 100000000
            num_lines = 1000

            chunk_size = int (round (num_lines/self.M))  # integer division
            print "doc size = ", num_lines, ", chunk size = ", chunk_size

            # number of total lines read from the file,
            ...
            for i in range (self.M):
                ...
                if (i == self.M-1): # if this is the last chunk
                    chunk_size = num_lines - linesRead

                # read the chunk_size number of lines
                content = [next(datafile) for x in xrange(chunk_size)]
    ...
    #/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
    # @NOTE@: changes maybe needed here for assignment #5
    #
    ...
    #/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
    # @NOTE@: changes will needed here for assignment #5
    #
    ...
    def shuffle_func (self):
        ...
        for i in range (self.M):
            ...
            #entries = sorted (csvfile, key=lambda x : int(x[0]))
            entries = sorted (csvfile, key=operator.itemgetter (0))
            ...
            groups = []
            uniquekeys = []
            for k, g in itertools.groupby (entries, key=operator.itemgetter (0)):
            ...
            for j in range (len (uniquekeys)):
                tempfile.write (str(uniquekeys[j]) + ", ")
                ...
                vals = [0.0,0.0,0]
                for m in groups[j]:
                    vals = [vals[0] + float(m[1]), vals[1] + float(m[2]), vals[2] + int(m[3])]
                tempfile.write (str(vals[0]) + ", " + str(vals[1]) + ", " + str(vals[2]) + "\n")
        ...
        rows = [[row[0], float(row[1]), float(row[2]), int(row[3])] for row in csvfile]
        
        # get the sorted list of entries from our csv file using
        # column 1 (0-based indexing used here) as the key to sort on
        # and we use traditional alphabetic order.  
        entries = sorted (rows, key=operator.itemgetter (0))
    ...
    #/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
    # @NOTE@: changes may not be needed here for assignment #5
    #
    ...
    def finalize_func (self):
        ...
        results.write("ID is defined by 4 chars for each the plug_id, household_id, and house_id.\n It will always have a total of 9 chars with leading zeros. \n ID, Average Work, Average Load.\n")
        for i in range (self.R):
    ...
    def run_iter (self, mf):
        ...
        try:
            ...
            print "*** Total Running Time for energy average ***  = ",  total_running_time
