...
class MR_Map ():
    ...
    def do_work (self):
        ...
        for line in content:
            ...
            key = [str(entry[4]), str(entry[5]), str(entry[6]).rstrip('\n')]
            ...
            if entry[3] == "0":
                intmed_key_val_list.append ({'token': ''.join(key), \
                    'sumWork': entry[2], 'sumLoad': 0, 'count': 1})
            elif entry[3] == "1":
                intmed_key_val_list.append ({'token': ''.join(key), \
                    'sumWork': 0, 'sumLoad': entry[2], 'count': 1})
